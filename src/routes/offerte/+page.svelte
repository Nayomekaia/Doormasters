...
<section class="page-wrapper" class:visible={isVisible}>
	...
	<section class="formulier-wrapper animate-slide-right">
		<form method="POST" class="offerte-formulier" use:enhance>
			...
			<section class="formulier-sectie animate-fade-in" style="animation-delay: 0.2s;">
				...
				<label class="input-wrapper">
					<p>Naam<span>*</span></p>
					<input type="text" name="naam" placeholder="Bijv. Jan" required />
				</label>

				<label class="input-wrapper">
					<p>Achternaam<span>*</span></p>
					<input type="text" name="achternaam" placeholder="Bijv. van Huizen" required />
				</label>

				<label class="input-wrapper">
					<p>Adres<span>*</span></p>
					<input type="text" name="adres" placeholder="Bijv. Kerkstraat 80" required />
				</label>

				<label class="input-wrapper">
					<p>Adres regel 2</p>
					<input type="text" name="adres2" placeholder="" />
				</label>

				<label class="input-wrapper">
					<p>Plaats<span>*</span></p>
					<input type="text" name="plaats" placeholder="Bijv. Alkmaar" required />
				</label>

				<label class="input-wrapper">
					<p>Staat/Regio/Provincie<span>*</span></p>
					<input type="text" name="staat" placeholder="Bijv. Noord-Holland" required />
				</label>

				<label class="input-wrapper">
					<p>Postcode<span>*</span></p>
					<input type="text" name="postcode" placeholder="Bijv. 1111AA" required />
				</label>

				<label class="input-wrapper">
					<p>Land<span>*</span></p>
					<input type="text" name="land" placeholder="Nederland" required />
				</label>

				<label class="input-wrapper">
					<p>Telefoonnummer<span>*</span></p>
					<input type="tel" name="telefoon" placeholder="Bijv. 0628763549" required />
				</label>

				<label class="input-wrapper">
					<p>E-mailadres<span>*</span></p>
					<input type="email" name="email" placeholder="Bijv. janvanhuizen@gmail.com" required pattern="^[^\s@]+@[^\s@]+\.[^\s@]+$" />
				</label>
			</section>
			...
			<section class="formulier-sectie-full animate-fade-in" style="animation-delay: 0.4s;">
				...
				<div class="button-wrapper animate-fade-in" style="animation-delay: 0.9s;">
					<Button type="submit" variant="silver">VERSTUREN</Button>
				</div>
			</section>
		</form>
	</section>
...
<style>
	...
	input::placeholder{color:#a0aec0;transition:opacity .2s ease;}
	...
	.radio-label:hover{border-color:var(--color-blue-dark);background:#f8fafc;box-shadow:0 2px 8px rgba(0,0,0,.08);transform:translateX(4px);}
	...
	.radio-label input[type='radio']:checked{transform:scale(1.1);}
	...
	.button-wrapper :global(button){transition:all .3s ease;padding:1rem 3rem;font-size:1rem;font-weight:600;box-shadow:0 4px 12px rgba(0,0,0,.15);}
	.button-wrapper :global(button:hover){transform:translateY(-3px);box-shadow:0 8px 20px rgba(0,0,0,.2);}

	@media(prefers-reduced-motion:reduce){*,*::before,*::after{animation-duration:.01ms !important;animation-iteration-count:1 !important;transition-duration:.01ms !important;}}
	@media(max-width:768px){.form-header{padding:2rem 1.5rem;} h2{font-size:1.5rem;} .formulier-sectie,.formulier-sectie-full{padding:2rem 1.5rem;}}
</style>
